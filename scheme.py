...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals != nil:
            if vals == nil:
                raise SchemeError('Argument \'vals\' has not enough elements.')
            ...
            vals = vals.second
        if vals != nil:
            raise SchemeError('Argument \'vals\' has too many elements.')
        # END PROBLEM 11
        return child
...
def do_and_form(expressions, env):
    ...
    if expressions.second == nil:
        return scheme_eval(expressions.first, env, True)
    if not scheme_falsep(scheme_eval(expressions.first, env)):
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            # END PROBLEM 14
        if scheme_falsep(test) and expressions.second is nil and clause.first is not 'else':
            return None
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        params = Pair(scheme_eval(expression.second.first, env), params)
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 16
    "*** REPLACE THIS LINE ***"
    MuProcedure.make_call_frame = lambda self, args, env: env.make_child_frame(self.formals, args)
...
def scheme_optimized_eval(expr, env, tail=False):
    """Evaluate Scheme expression EXPR in environment ENV. If TAIL call, returns an
